...
const ListItem = ( {
	 ...
	 * @param {boolean} isSeconadaryCall
	 */
	const onMerge = ( isMergeWithBottom, isSeconadaryCall = false ) => {
		if ( ! isMergeWithBottom ) {
			if ( isSecondaryEnabled && isSeconadaryCall ) {
				setItem( index, {
					primaryText: `${ primaryText }${ editedText }`,
					secondaryText: '',
				} );
				onFocus( index, false, primaryText.length );
			} else {
