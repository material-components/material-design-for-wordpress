...
describe( 'blocks: material/hand-picked-posts', () => {
	...
	it( 'should be inserted', async () => {
		await insertBlock( 'Hand-picked Posts' );
	...
	it( 'should have a "Styles" radio selection attribute with Masonry being the default', async () => {
		await insertBlock( 'Hand-picked Posts' );
		await selectBlockByName( 'material/hand-picked-posts' );
	...
	it( 'should have a Post selection searchbox', async () => {
		await insertBlock( 'Hand-picked Posts' );
		await selectBlockByName( 'material/hand-picked-posts' );
		expect(
			await page.$x(
				"//label[contains(text(), 'Search for posts to display')]/following-sibling::input"
			)
		).toHaveLength( 1 );
	} );
} );
