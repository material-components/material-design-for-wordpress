...
describe( 'blocks: material/recent-posts', () => {
	...
	it( 'should have an "Outlined" and "Filled" radio selection attribute with default being "Filled"', async () => {
		await insertBlock( 'Recent Posts (Material)' );
		await selectBlockByName( 'material/recent-posts' );

		expect(
			await page.$x(
				"//label[contains(text(), 'Outlined')]/preceding-sibling::span/input[@type = 'radio' and not(@checked)]"
			)
		).toHaveLength( 1 );
	} );
